...
public class Summarization
{
        ...
        if(words.size() > 3) {
            for(int i = 0; i < 3; i++) {
                ...
                for(int h = 0; h < words.size(); h++) {
                    Integer freq = (Integer)frequency.get(h);
                    if(freq.intValue() > greatest) {
                        greatest = freq.intValue();
                        greatestIndex = h;
                    }
                    if(i != 4)
                        search += " ";
                }
                search += words.get(greatestIndex);
                frequency.remove(greatestIndex);
                frequency.add(new Integer(0), greatestIndex);
            }
        } else {
    ...
    private void setSentences() {
        ...
        for(int h = 0; h < numSentences; h++) {
            ...
            if(index > 0 && !Character.isUpperCase(text.charAt(index - 1))) {
                if(index == 1) {
                    if(isEnd(index)) {
                        sentences[h] = text.substring(startIndex, index+1);
                        startIndex = index+1;
                    } else
                    h--;
                } else if(index == 2) {
    ...
    public String[] shorten(String[] sent) {
